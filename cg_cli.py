    ...
    with open(input_file, 'r') as fp:
        ...
        while line:
            ...
            if line[0] == 'resetCanvas':
                ...
                dx = int(line[2])
                dy = int(line[3])
                pre_list = item_dict[item_id][1]
                new_list = alg.translate(pre_list,dx,dy)
                item_dict[item_id][1] = new_list
            elif line[0] == 'rotate':
                item_id = line[1]
                x = int(line[2])
                y = int(line[3])
                r = int(line[4])
                if item_dict[item_id]=='ellipse':
                    pass
                else:
                    pre_list=item_dict[item_id][1]
                    new_list=alg.rotate(pre_list, x, y, 360-r)
                    item_dict[item_id][1]=new_list
            elif line[0] == 'scale':
                item_id = line[1]
                x = int(line[2])
                ...
                s = float(line[4])
                pre_list = item_dict[item_id][1]
                new_list = alg.scale(pre_list,x,y,s)
                item_dict[item_id][1] = new_list
            elif line[0] == 'clip':
                item_id = line[1]
                x0 = int(line[2])
                ...
                algorithm = line[6]
                pre_list = item_dict[item_id][1]
                new_list = alg.clip(pre_list, x0, y0, x1, y1, algorithm)
                item_dict[item_id][1] = new_list
